# QQ群年度报告分析器

> 📌 **本项目基于 [QQgroup-annual-report-analyzer](https://github.com/ZiHuixi/QQgroup-annual-report-analyzer) 二次开发**，在原项目基础上新增了互动关系网络、群聊CP分析、气氛担当、语言指纹、时间脉搏等分析维度，重构了 UI 样式为原神清新风格（1080px 宽幅排版），并优化了图片导出、停用词过滤、统计口径说明等功能。向原作者致敬。

QQ 群聊天记录分析工具，生成精美的年度总结报告。原神清新风格，1080px 宽幅排版，支持一键导出分享长图。

#### ❗请注意本项目代码基于vibe coding开发，ai含量高，请酌情使用。
#### ❗该项目仍处于开发阶段，某些迭代中可能功能未完善且存在大量 Bug ，还是请酌情使用，稳定版会稍后发布release, 在线版网站正在制作中，欢迎大家提出建议。

## ✨ 核心特性

- 📊 **智能词频统计**：基于 jieba 分词的高级文本分析，自动过滤停用词和历史昵称
- 🔍 **新词发现**：自动识别群聊专属新词
- 📈 **多维度排行榜**：发言量、活跃度、表情包、夜猫子等多个维度
- 🕸️ **互动关系网络**：Canvas 力导向图可视化群友互动关系，节点大小=发言量，连线=回复/@
- 💞 **群聊CP分析**：最佳拍档（互相回复最多）+ 单向奔赴（一方互动远超另一方）
- 🔥 **气氛担当**：热场王（发言后引发最多回复）与冷场王（发言后最少人接话）
- 🗣️ **语言指纹**：个人口头禅（突增度排序）+ 标点狂魔（感叹号/问号/省略号）
- 📅 **时间脉搏**：24小时活跃时段 + 月度趋势 + 年度大事件（突发热词识别）
- 🎨 **原神清新风格**：淡蓝米白渐变、金色装饰、1080px 宽幅排版、Q版角色装饰
- 🖼️ **一键导出长图**：Playwright 高清截图，样式版本号自动管理缓存
- 📝 **统计口径透明**：每个模块附口径说明，报告末尾附完整统计解释
- 🤖 **AI 智能点评**：集成 OpenAI API，提供 AI 年度总结（可选）
- 🎯 **交互式选词**：Web 界面支持从热词列表中自主选择展示词汇，未配AI时自动用前10热词
- 💾 **数据持久化**：JSON 文件存储，报告自动保存可随时回看
- 📜 **历史记录管理**：随时查看、搜索、删除历史报告
- ⚙️ **高度可定制**：丰富的配置参数，满足不同需求

## 📸 效果预览

![年度报告样例](docs/sample-report.png)

## 🚀 快速开始（3步启动）

### 📋 第1步：安装必需软件

请确保已安装以下软件：

1. **Python 3.8+** （必需）
   - 下载：[python.org](https://www.python.org/downloads/)
   - 安装时勾选"Add Python to PATH"

2. **Node.js 16+** （必需）
   - 下载：[nodejs.org](https://nodejs.org/)

3. **qq-chat-exporter** （必需）
   - 下载：[qq-chat-exporter](https://github.com/shuakami/qq-chat-exporter)
   - 使用该项目导出 QQ 群聊天记录为 JSON

### 🎯 第2步：启动服务

#### Windows 用户（推荐使用一键启动）

**首次运行：**

1. 双击运行 `start.bat`
2. 脚本会自动创建配置文件并提示你配置
3. 编辑配置文件（默认配置即可用）
4. 再次运行 `start.bat` 即可启动

**后续运行：**

直接双击 `start.bat` 即可启动所有服务。

#### macOS / Linux 用户
mac默认可能5000端口被占用的话换一个端口

1. **启动后端**（新开一个终端）：
   ```bash
   cd backend
   pip install -r requirements.txt
   PORT=5000 python app.py
   # 如果 5000 端口被占用，会自动尝试 5001
   ```

2. **启动前端**（再开一个终端）：
   ```bash
   cd frontend
   npm install
   npm run dev
   ```

3. **配置前端代理**（如果后端不是 5000 端口）：
   - 编辑 `frontend/vite.config.js`
   - 修改 `proxy.target` 为实际后端地址（如 `http://localhost:5001`）

### ✅ 第3步：访问应用

启动成功后，浏览器访问：

- **前端界面**：http://localhost:5173
- **后端API**：http://localhost:5000（或你配置的端口）

就是这么简单！🎉

## ⚙️ 配置说明

首次运行时会自动创建两个配置文件：

### backend/.env（Web 模式配置）

```env
# 存储模式：JSON 文件存储，无需数据库
STORAGE_MODE=json

# Flask 配置
FLASK_SECRET_KEY=your_secret_key
FLASK_PORT=5000
DEBUG=false

# CORS 配置
ALLOWED_ORIGINS=http://localhost:5173,http://localhost:5000

# OpenAI API（用于AI功能，可选）
OPENAI_API_KEY=sk-...
OPENAI_BASE_URL=https://api.openai.com/v1
OPENAI_MODEL=gpt-3.5-turbo
```

**数据存储说明：**

- 数据存储在 `runtime_outputs/reports_db/` 目录
- 每个报告一个 JSON 文件，自动创建索引
- 适合个人本地使用，易于备份和迁移

## 📖 使用方式

通过浏览器访问可视化界面，提供最友好的交互体验。

**Windows 用户：**
1. 双击运行 `start.bat` 启动服务
2. 浏览器访问 http://localhost:5173
3. 上传 QQ 群聊天记录 JSON 文件
4. 选择想要展示的热词（未配AI时自动用前10热词）
5. 生成并查看精美报告，可一键导出分享长图

**macOS / Linux 用户：**
1. 按照"第2步"中的说明启动前后端服务
2. 浏览器访问 http://localhost:5173
3. 上传 QQ 群聊天记录 JSON 文件
4. 选择想要展示的热词
5. 生成并查看精美报告

## 📊 生成的报告包含

- 🏷️ **封面**：群名、消息总数、年度关键词
- 🔥 **年度热词榜**：群聊最热门的词汇（可自定义选择），附AI锐评（可选）
- 📖 **热词档案**：每个热词的使用者排行，2列卡片布局
- 👑 **荣誉殿堂**：多维度排行榜（发言量、表情包达人、夜猫子/早起人、图片分享达人等）
- ⏰ **时间脉搏**：24小时活跃时段分布 + 月度活跃趋势 + 年度大事件（突发热词）
- 🕸️ **关系网络**：Canvas 力导向互动图谱，展示群友回复/@关系
- 💞 **群聊CP**：最佳拍档（互相互动最多）+ 单向奔赴（一方默默互动）
- 🔥 **气氛担当**：热场王与冷场王（发言后5分钟内非本人回复数均值）
- 🗣️ **语言指纹**：个人口头禅（综合得分排序）+ 标点狂魔（感叹号/问号/省略号）
- 📋 **统计说明**：报告末尾附完整统计口径解释
- 🖼️ **一键导出**：1080px 宽高清 PNG 长图，适合分享

## 🛠️ 技术栈

- **后端**：Flask, Python 3.8+
- **前端**：Vue 3, Vite
- **分析引擎**：jieba（中文分词）
- **可视化**：Canvas 力导向图、CSS 图表
- **图片生成**：Playwright
- **数据存储**：JSON 文件
- **AI 功能**：OpenAI API（可选）

## 💾 数据存储

采用 JSON 文件存储：

- 数据保存在 `runtime_outputs/reports_db/` 目录
- 每个报告一个 JSON 文件
- 自动创建索引文件用于快速查询
- 图片缓存在 `runtime_outputs/reports_db/images/` 目录
- 易于备份和迁移（直接拷贝目录即可）

## 📁 项目结构

```
group-pulse/
├── start.bat              # 一键启动脚本（Windows）
├── README.md              # 本文档
├── analyzer.py            # 分析核心逻辑（含停用词、互动图谱、口头禅等）
├── image_generator.py     # 图片导出功能（Playwright 截图）
├── utils.py               # 工具函数（时间戳解析、JSON加载）
├── logger.py              # 日志模块
├── backend/               # Web 后端
│   ├── app.py            # Flask 应用（含图片生成、缓存、样式版本号）
│   ├── json_storage.py   # JSON 文件存储服务
│   ├── .env.example      # 环境变量模板
│   └── requirements.txt  # Python 依赖
├── frontend/              # Web 前端
│   ├── src/
│   │   ├── App.vue       # 主应用（上传/历史列表）
│   │   ├── Report.vue    # 报告展示页
│   │   ├── main.js       # 入口
│   │   ├── style.css     # 全局样式
│   │   ├── report-styles.css  # 报告页样式（原神清新风格）
│   │   ├── assets/       # 装饰图片（Q版角色、表情包）
│   │   ├── composables/  # 组合式函数
│   │   └── templates/
│   │       └── classic.vue   # classic 报告模板（含力导向图）
│   ├── package.json      # NPM 配置
│   └── vite.config.js    # Vite 配置
└── templates/             # HTML 导出模板
    ├── report_template.html   # 导出用 HTML 模板（原神清新风格）
    └── assets/           # 导出模板装饰图片
```

## 🎯 使用建议

1. **首次使用**
   - 双击 `start.bat` 启动，默认配置即可使用
   - 如需 AI 功能，配置 OpenAI API Key；未配置时自动用前10热词

2. **日常使用**
   - **Windows**：双击 `start.bat` 启动
   - **macOS/Linux**：手动启动前后端
   - 在浏览器中上传聊天记录，生成报告
   - 关闭服务窗口即停止服务

3. **导出分享**
   - 报告页左下角点击"生成图片分享"，导出 1080px 宽高清长图
   - 样式修改后会自动使缓存失效（样式版本号机制）

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📄 许可证

AGPL-3.0 License

本项目采用 GNU Affero General Public License v3.0 开源协议。

**重要提示**：如果您修改本软件并通过网络提供服务，必须向用户提供修改后的完整源代码。

## 🙏 致谢

- [qq-chat-exporter](https://github.com/shuakami/qq-chat-exporter) - QQ 聊天记录导出工具
- [jieba](https://github.com/fxsjy/jieba) - 中文分词库

## 📮 联系方式

- **原项目**：[QQgroup-annual-report-analyzer](https://github.com/ZiHuixi/QQgroup-annual-report-analyzer) by [@ZiHuixi](https://github.com/ZiHuixi) & [@Jingkun Yu](https://github.com/yujingkun1)
- **本仓库**：https://github.com/qingyang-o/group-pulse



---

**注意**：本项目仅供学习和个人使用，请遵守相关法律法规和平台服务条款。
